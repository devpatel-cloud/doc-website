# DocVault V2 — Security Audit & Hardening Report

This document details the defense-in-depth security audit performed on **DocVault V2**, outlining identified risks, mitigations, empirical test results, residual risks, and recommendations for future production operations.
...
|---|---|---|---|---|---|---|
| **1** | **Timing Attack on Admin Authentication** | **HIGH** | Replaced standard string comparison with `secrets.compare_digest` in FastAPI backend. | Verified login API with valid and invalid passwords via test suite. | Low. Brute force attempts still possible over long timeframes. | Implement Fail2Ban or IP firewall blocking at server level. |
...
| **8** | **Storage Exhaustion / DoS** | **MEDIUM** | Enforced 100MB file size limit (`MAX_FILE_SIZE_MB`), 1GB minimum disk space threshold check (`MIN_FREE_DISK_GB`), and atomic temp upload cleanup. | Attempted oversized upload and verified free storage check in `app/main.py`. | Low. Rapid multi-user parallel uploads exhausting free threshold simultaneously. | Implement disk quota per directory / user role. |
